...
| `IMonnifyDisbursementsClient.GetBulkTransfersAsync` | `GET /api/v2/disbursements/bulk?sourceAccountNumber=&pageNo=&pageSize=` | **Implemented** | We document this as `GET /api/v2/disbursements/bulk/transactions`, which 404s in the sandbox — the correct path has no `/transactions` suffix |
...
| `IMonnifyDisbursementsClient.GetWalletBalanceAsync` | `GET /api/v2/disbursements/wallet-balance?accountNumber=` | **Implemented** | Sandbox returns plain numeric balances; our docs show them as quoted strings — `decimal` properties accept either via `JsonNumberHandling.AllowReadingFromString` |
...
| `IMonnifyBillsClient.GetBillerCategoriesAsync` | `GET /api/v1/vas/bills-payment/biller-categories?page=&size=` | **Implemented** | Sandbox-verified. We currently document 1-based paging (`page` defaults to `1`); the real sandbox is 0-based like every other paginated list in this SDK — passing `page=1` against the real API silently skips the first page. Also: the real default `size` is `20`, not the `10` our docs show for this endpoint specifically |
| `IMonnifyBillsClient.GetBillersAsync` | `GET /api/v1/vas/bills-payment/billers?page=&size=&category_code=` | **Implemented** | Sandbox-verified. Same 0-based paging correction as above |
| `IMonnifyBillsClient.GetBillerProductsAsync` | `GET /api/v1/vas/bills-payment/biller-products?biller_code=&page=&size=` | **Implemented** | Sandbox-verified. We currently document `categories`/`billers` as arrays on each product; the real sandbox response uses singular `category`/`biller` objects instead — confirmed with both an airtime and a data product. `biller_code` works with both a plain code (`AIRTEL`) and a `biller-`-prefixed slug (`biller-dstv`), matching whichever form `GetBillersAsync` returned that biller's own `code` as |
| `IMonnifyBillsClient.ValidateCustomerAsync` | `POST /api/v1/vas/bills-payment/validate-customer` | **Implemented** | Sandbox-verified for both the "no reference required" case (airtime) and the "reference required" case (an electricity prepaid product) — the latter revealed `validationReference` actually lives inside `vendInstruction`, not at the top level as our docs prose implied, and that `minAmount`/`maxAmount` can appear at the top level for amount-constrained OPEN-priced products, neither of which our docs' only sample (a FIXED-price airtime product) showed |
| `IMonnifyBillsClient.VendAsync` | `POST /api/v1/vas/bills-payment/vend` | **Implemented** | Sandbox-verified end-to-end (airtime top-up, `requestSuccessful: true`, full result shape matches the model). Registered with automatic retry disabled, same reasoning as Disbursements — an ambiguous failure must be resolved via `RequeryAsync` with the same reference, not retried with the same one |
| `IMonnifyBillsClient.RequeryAsync` | `GET /api/v1/vas/bills-payment/requery?reference=` | **Implemented** | Sandbox-verified — confirmed it returns the same response shape as `VendAsync`, and confirmed the not-found error shape (`responseCode: "99"`, `"Transaction not found for reference: ..."`) |
